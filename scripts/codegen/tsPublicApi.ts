...
const commonProps = nodes.reduce((acc, { props }) => acc.intersection(props), nodes[0].props);
...
const content = dedent/* ts */`
    ...
    export const publicNodeProperties: PublicNodeProperties = /* @__PURE__ */ Object.freeze(new Map([
        ${nodes
            /* eslint-disable @stylistic/indent */
            .map(({ name, props }) => {
                const difference = props.difference(commonProps);

                if (difference.size === 0) {
                    return `/* @__PURE__ */ f(${JSON.stringify(name)})`;
                }
                ...
                return `/* @__PURE__ */ g(${JSON.stringify(name)}, ${rest})`;
            })
            .join(",\n")
            /* eslint-enable @stylistic/indent */
        }
...
const markerMapContent = dedent`
    ...
    export const markerMap: ReadonlyMap<string, string> = /* @__PURE__ */ Object.freeze(new Map(${JSON.stringify(Array.from(analyzer.getMarkerMap()))}));
`;
