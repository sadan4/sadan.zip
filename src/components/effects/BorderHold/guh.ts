...
// some stolen from https://github.com/vercel/satori/blob/main/src/builder/border-radius.ts
...
function resolveRadius(
    ...
    if (typeof v === "string") {
        ...
        return [
            singleValue,
            [
                Math.min(lengthToNumber(sides[0], fontSize, width, style, true) ?? error(), width),
                Math.min(
                    lengthToNumber(sides[1], fontSize, height, style, true) ?? error(),
                    height,
                ),
            ],
        ];
...
function radiusZeroOrNull(_radius?: [number, number]) {
...
export function borderRadiusToPath(
    {
        left,
        top,
        width,
    ...
    let singleAbsValueTopLeftCorner: boolean;
    let singleAbsValueTopRightCorner: boolean;
    let singleAbsValueBottomLeftCorner: boolean;
    let singleAbsValueBottomRightCorner: boolean;
