...
import type { IEmbeddedLanguagesMap } from "../main";
...
export class BasicScopeAttributesProvider {
    ...
    private readonly _embeddedLanguagesMatcher: ScopeMatcher<number>;
...
class ScopeMatcher<TValue> {
    ...
    constructor(values: [ScopeName, TValue][]) {
        if (values.length === 0) {
            ...
            // create the regex
            const escapedScopes = values.map(([scopeName, _value]) => escapeRegExpCharacters(scopeName));
