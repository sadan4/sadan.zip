...
import type { AttributedScopeStack, Grammar, Injection, LineTokens, StateStackImpl } from "./grammar";
import { DebugFlags, UseOnigurumaFindOptions } from "../debug";
import { disposeOnigString, FindOption, type IOnigCaptureIndex, type OnigString } from "../onigLib";
import { BeginEndRule, BeginWhileRule, CaptureRule, CompiledRule, endRuleId, MatchRule, Rule, type RuleId, whileRuleId } from "../rule";
        ...
        if (matchedRuleId === endRuleId) {
            ...
            if (_rule instanceof BeginEndRule) {
                ...
                if (pushedRule.endHasBackReferences) {
                    stack = stack.withEndRule(pushedRule.getEndWithResolvedBackReferences(
                        lineText.content,
                        captureIndices,
                    ));
                }
                ...
                if (pushedRule.whileHasBackReferences) {
                    stack = stack.withEndRule(pushedRule.getWhileWithResolvedBackReferences(
                        lineText.content,
                        captureIndices,
                    ));
                }
...
function prepareRuleSearch(rule: Rule, grammar: Grammar, endRegexSource: string | null, allowA: boolean, allowG: boolean): { ruleScanner: CompiledRule;
    findOptions: number; } {
...
function prepareRuleWhileSearch(rule: BeginWhileRule, grammar: Grammar, endRegexSource: string | null, allowA: boolean, allowG: boolean): { ruleScanner: CompiledRule<RuleId | typeof whileRuleId>;
    findOptions: number; } {
    if (UseOnigurumaFindOptions) {
