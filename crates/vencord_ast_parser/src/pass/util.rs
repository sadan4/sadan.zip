...
impl<'a, 'ast: 'a, State> Ctx<'a, 'ast, State> {
	...
	pub fn eval_template(&self, t: &TemplateLiteral<'ast>) -> Atom<'ast> {
		...
		let expr_strs = t.expressions.iter().map(|e| {
			e.evaluate_value_to_string(self)
				.unwrap()
		});
...
impl<'a, 'ast: 'a, State> MayHaveSideEffectsContext<'ast>
	for Ctx<'a, 'ast, State>
{
	...
	fn manual_pure_functions(
		&self,
		_callee: &oxc::ast::ast::Expression,
	) -> bool {
		false
	}
