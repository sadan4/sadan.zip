...
fn mk_graph(acyclicer: Option<&str>) -> Graph<GraphLabel, NodeLabel, EdgeLabel> {
...
fn run_creates_multi_edge_where_necessary() {
    run_for_each_acyclicer(|ac| {
        ...
        let ab = g.out_edges_to("a", "b").unwrap_or_default();
        let ba = g.out_edges_to("b", "a").unwrap_or_default();
        assert!(
            ab.len() == 2 || ba.len() == 2,
