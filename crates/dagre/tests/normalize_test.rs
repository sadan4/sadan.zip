...
use dagre::normalize;
use dagre::types::{Dummy, EdgeLabel, GraphLabel, NodeLabel, Point};
...
fn run_splits_two_layer_edge_into_two_segments() {
    ...
    let chains = g.graph().unwrap().dummy_chains.clone().unwrap_or_default();
...
fn undo_collects_assigned_coordinates_into_points() {
    ...
    normalize::undo(&mut g);
    let pts = g.edge("a", "b").unwrap().points.clone().unwrap_or_default();
...
fn undo_sets_coords_and_dims_when_short_edge_has_label() {
    ...
    assert_eq!((e.x, e.y, e.width, e.height), (Some(50.0), Some(60.0), 20.0, 10.0));
}
...
fn undo_restores_multi_edges() {
    ...
    let bar = g.edge_full("a", "b", Some("bar")).unwrap();
    let foo = g.edge_full("a", "b", Some("foo")).unwrap();
    assert_eq!(
