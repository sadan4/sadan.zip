...
use dagre::rank;
use dagre::types::{EdgeLabel, GraphLabel, NodeLabel, Ranker};

fn make_graph() -> Graph<GraphLabel, NodeLabel, EdgeLabel> {
    let mut g: Graph<GraphLabel, NodeLabel, EdgeLabel> =
        Graph::with_opts(GraphOpts::directed().multigraph().compound());
...
fn bench_ranker(c: &mut Criterion, name: &str, base: &Graph<GraphLabel, NodeLabel, EdgeLabel>, ranker: Ranker) {
...
fn bench_layout(c: &mut Criterion, name: &str, base: &Graph<GraphLabel, NodeLabel, EdgeLabel>) {
    c.bench_function(name, |b| {
        b.iter_batched(
            || clone_graph(base),
...
fn clone_graph(
    ...
    for e in g.edges() {
        if let Some(l) = g.edge_obj(&e) {
            out.set_edge_named(e.v.clone(), e.w.clone(), l.clone(), e.name.clone());
        }
    }
    out
}
...
fn small_benches(c: &mut Criterion) {
    ...
    bench_ranker(c, "network-simplex ranker (small)", &g, Ranker::NetworkSimplex);
...
fn large_benches(c: &mut Criterion) {
    ...
    bench_ranker(c, "network-simplex ranker (large)", &g, Ranker::NetworkSimplex);
