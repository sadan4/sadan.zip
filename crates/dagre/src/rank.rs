...
use crate::types::{EdgeLabel, GraphLabel, NodeLabel, Ranker};
...
pub mod util_rank {
    ...
    pub fn slack(graph: &Graph<GraphLabel, NodeLabel, EdgeLabel>, e: &Edge) -> i32 {
        let v_rank = graph.node(&e.v).and_then(|n| n.rank).unwrap_or(0);
        let w_rank = graph.node(&e.w).and_then(|n| n.rank).unwrap_or(0);
        let minlen = graph.edge_obj(e).map(|l| l.minlen).unwrap_or(1);
...
pub mod feasible_tree {
    ...
    fn tight_tree(tree: &mut Tree, graph: &Graph<GraphLabel, NodeLabel, EdgeLabel>) -> usize {
        ...
        loop {
            ...
            for v in tree.nodes() {
                ...
                for e in es {
                    ...
                    if !tree.has_node(&w) && super::util_rank::slack(graph, &e) == 0 {
                        tree.set_node(w.clone(), TreeNode::default());
                        tree.set_edge(v.clone(), w.clone(), TreeEdge::default());
                    }
                }
            }
...
pub mod network_simplex {
    use super::feasible_tree::{Tree, TreeEdge, TreeNode};
    use super::util_rank::{longest_path, slack};
    use super::*;
    ...
    pub fn init_cut_values(tree: &mut Tree, graph: &Graph<GraphLabel, NodeLabel, EdgeLabel>) {
        let ns = tree.nodes();
        ...
        if let Some(es) = graph.node_edges(child) {
            for e in es {
                ...
                if other != parent {
                    ...
                    let other_w = graph.edge_obj(&e).map(|l| l.weight).unwrap_or(0.0);
                    cut += if points_to_head { other_w } else { -other_w };
                    if tree.has_edge(child, &other) {
                        if let Some(other_cv) = tree.edge(child, &other).and_then(|e| e.cutvalue) {
                            cut += if points_to_head { -other_cv } else { other_cv };
                        }
                    }
                }
            }
    ...
    pub fn leave_edge(tree: &Tree) -> Option<Edge> {
        tree.edges()
            .into_iter()
            .find(|e| tree.edge_obj(e).and_then(|l| l.cutvalue).unwrap_or(0.0) < 0.0)
    }
    ...
    pub fn enter_edge(
        ...
        let v_label = tree.node(&v).cloned().unwrap_or_default();
        let w_label = tree.node(&w).cloned().unwrap_or_default();
        ...
        let candidates: Vec<Edge> = graph
            ...
            .filter(|edge| {
                let v_desc = is_descendant(
                    &tree.node(&edge.v).cloned().unwrap_or_default(),
                    &tail_label,
                );
                let w_desc = is_descendant(
                    &tree.node(&edge.w).cloned().unwrap_or_default(),
                    &tail_label,
                );
    ...
    fn update_ranks(tree: &Tree, graph: &mut Graph<GraphLabel, NodeLabel, EdgeLabel>) {
        ...
        for v in pre.into_iter().skip(1) {
            ...
            let (minlen, flipped) = if graph.has_edge(&v, &parent) {
                (
                    graph.edge(&v, &parent).map(|l| l.minlen).unwrap_or(1),
                    false,
                )
            } else {
                (
                    graph.edge(&parent, &v).map(|l| l.minlen).unwrap_or(1),
                    true,
                )
            };
...
pub fn rank(graph: &mut Graph<GraphLabel, NodeLabel, EdgeLabel>) {
    ...
    let ranker = graph.graph().and_then(|g| g.ranker).unwrap_or(Ranker::NetworkSimplex);
    match ranker {
