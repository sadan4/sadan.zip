...
fn normalize_edge(graph: &mut Graph<GraphLabel, NodeLabel, EdgeLabel>, e: &Edge) {
    let v_rank = graph
        .node(&e.v)
        .and_then(|n| n.rank)
        .expect("v has rank");
    ...
    let edge_label = graph.edge_obj(e).cloned().unwrap_or_default();
...
pub fn undo(graph: &mut Graph<GraphLabel, NodeLabel, EdgeLabel>) {
    ...
    for start in chains {
        ...
        loop {
            ...
            let w = match graph.successors(&v).and_then(|s| s.into_iter().next()) {
                Some(w) => w,
                None => break,
            };
            graph.remove_node(&v);
