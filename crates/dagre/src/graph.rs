...
impl<G, N, E> Graph<G, N, E> {
    ...
    pub fn with_opts(opts: GraphOpts) -> Self {
        ...
        if g.is_compound {
            g.children.insert(GRAPH_NODE.to_string(), BTreeSet::new());
        }
        g
    }
    ...
    pub fn set_default_node_label<F: Fn(&str) -> N + 'static>(&mut self, f: F) -> &mut Self {
        self.default_node_label = Some(Box::new(f));
        self
    }
    pub fn set_default_edge_label<F: Fn(&Edge) -> E + 'static>(&mut self, f: F) -> &mut Self {
    ...
    pub fn set_node(&mut self, v: impl Into<String>, label: N) -> &mut Self {
        ...
        self.node_labels.insert(v.clone(), label);
        self.node_index.insert(v.clone(), self.node_order.len());
        self.node_order.push(v.clone());
        self.in_edges.insert(v.clone(), BTreeMap::new());
        self.out_edges.insert(v.clone(), BTreeMap::new());
        self.preds.insert(v.clone(), HashMap::new());
        self.sucs.insert(v.clone(), HashMap::new());
        if self.is_compound {
            ...
            self.children
                ...
                .or_default()
                .insert(v.clone());
            self.children.insert(v.clone(), BTreeSet::new());
        }
        self
    }
    ...
    pub fn remove_node(&mut self, v: &str) {
        ...
        if self.is_compound {
            ...
            for c in &cs {
                ...
                self.parent.insert(c.clone(), GRAPH_NODE.to_string());
                self.children
                    .entry(GRAPH_NODE.to_string())
                    .or_default()
                    .insert(c.clone());
        ...
        let out_es: Vec<Edge> = self
            ...
            .map(|m| m.values().cloned().collect())
            .unwrap_or_default();
        for e in in_es.into_iter().chain(out_es.into_iter()) {
        ...
        N: Default,
    {
        ...
        if parent.is_some() {
            ...
            while let Some(a) = ancestor {
                if a == v {
                    panic!("Setting {} as parent of {} would create a cycle", new_parent, v);
                }
        ...
        self.parent.insert(v.to_string(), new_parent.clone());
        self.children
            .entry(new_parent)
            .or_default()
            .insert(v.to_string());
    ...
    pub fn set_edge(&mut self, v: impl Into<String>, w: impl Into<String>, label: E)
    where
        N: Default,
        E: Default,
    {
    ...
    pub fn set_edge_default(&mut self, v: impl Into<String>, w: impl Into<String>)
    where
        N: Default,
        E: Default,
    {
    ...
    where
        ...
        E: Default,
    {
        self.set_edge_full(e.v.clone(), e.w.clone(), e.name.clone(), Some(label));
    }
        ...
        E: Default,
    {
        ...
        self.edge_labels.insert(id.clone(), label);
        self.edge_objs.insert(id.clone(), edge.clone());
        self.edge_index.insert(id.clone(), self.edge_order.len());
        ...
        *self.sucs.get_mut(&v).unwrap().entry(w.clone()).or_insert(0) += 1;
        *self.preds.get_mut(&w).unwrap().entry(v.clone()).or_insert(0) += 1;
    }
    ...
    pub fn edge_full(&self, v: &str, w: &str, name: Option<&str>) -> Option<&E> {
        let id = self.edge_id(v, w, name);
        self.edge_labels.get(&id)
    }
    pub fn edge_full_mut(&mut self, v: &str, w: &str, name: Option<&str>) -> Option<&mut E> {
        let id = self.edge_id(v, w, name);
    ...
    pub fn in_edges_from(&self, v: &str, u: &str) -> Option<Vec<Edge>> {
        let m = self.in_edges.get(v)?;
        Some(m.values().filter(|e| e.v == u).cloned().collect())
    }
    ...
    pub fn out_edges_to(&self, v: &str, w: &str) -> Option<Vec<Edge>> {
        let m = self.out_edges.get(v)?;
        Some(m.values().filter(|e| e.w == w).cloned().collect())
    }
    ...
    pub fn sources(&self) -> Vec<String> {
        self.node_order
            .iter()
            .filter(|v| self.preds.get(*v).map(|m| m.is_empty()).unwrap_or(true))
            .cloned()
            .collect()
    }
        ...
        E: Default,
    {
        for i in 1..path.len() {
            ...
            self.set_edge_full(path[i - 1].to_string(), path[i].to_string(), None, None);
        }
    }
    ...
    pub fn sinks(&self) -> Vec<String> {
        self.node_order
            .iter()
            .filter(|v| self.sucs.get(*v).map(|m| m.is_empty()).unwrap_or(true))
            .cloned()
            .collect()
    }
}
...
pub mod alg {
    ...
    pub fn postorder<G, N, E>(g: &Graph<G, N, E>, starts: &[String]) -> Vec<String> {
        ...
        fn strong_connect<G, N, E>(s: &mut State<G, N, E>, v: &str) {
            ...
            for w in s.g.successors(v).unwrap_or_default() {
                if !s.indices.contains_key(&w) {
                    ...
                    s.lowlinks.insert(v.to_string(), lv.min(lw));
                } else if s.on_stack.contains(&w) {
                    let iw = *s.indices.get(&w).unwrap();
                    let lv = *s.lowlinks.get(v).unwrap();
                    s.lowlinks.insert(v.to_string(), lv.min(iw));
                }
            }
    ...
    pub fn preorder<G, N, E>(g: &Graph<G, N, E>, starts: &[String]) -> Vec<String> {
        let mut visited: HashSet<String> = HashSet::new();
        let mut result: Vec<String> = Vec::new();
        for s in starts {
            if !g.has_node(s) {
...
mod tests {
    ...
    fn multigraph_named_edges() {
        let mut g: Graph<(), i32, i32> = Graph::with_opts(GraphOpts::directed().multigraph());
        g.set_node("a", 1);
        g.set_node("b", 2);
    ...
    fn compound_parent_children() {
        let mut g: Graph<(), i32, i32> = Graph::with_opts(GraphOpts::directed().compound());
        g.set_node("p", 0);
        g.set_node("c", 1);
