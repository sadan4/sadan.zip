...
use crate::types::{Align, BorderType, EdgeLabel, GraphLabel, LabelPos, NodeLabel, RankAlign};
use crate::util;
...
fn position_y(graph: &mut Graph<GraphLabel, NodeLabel, EdgeLabel>) {
    ...
    let (rank_sep, rank_align) = {
        let gl = graph.graph().cloned().unwrap_or_default();
        (gl.ranksep.unwrap_or(50.0), gl.rank_align.unwrap_or(RankAlign::Center))
    };
    let mut prev_y = 0.0_f64;
    for layer in layering {
        let max_h = layer
            .iter()
            .map(|v| graph.node(v).map(|n| n.height).unwrap_or(0.0))
            .fold(0.0f64, f64::max);
        for v in &layer {
            let h = graph.node(v).map(|n| n.height).unwrap_or(0.0);
...
pub mod bk {
    pub use super::{
        ...
        position_x, vertical_alignment, AlignmentResult, Conflicts, PositionMap,
    };
}

pub fn add_conflict(c: &mut Conflicts, v: &str, w: &str) {
    let (a, b) = if v > w { (w, v) } else { (v, w) };
    c.entry(a.to_string()).or_default().insert(b.to_string());
}

pub fn has_conflict(c: &Conflicts, v: &str, w: &str) -> bool {
    let (a, b) = if v > w { (w, v) } else { (v, w) };
    c.get(a).map(|s| s.contains(b)).unwrap_or(false)
}
    ...
    for layer in layering {
        if let Some(prev_layer) = prev {
            ...
            for (i, v) in layer.iter().enumerate() {
                ...
                let k1 = match &w_opt {
                    Some(w) => graph.node(w).and_then(|n| n.order).unwrap_or(0),
                    None => prev_len,
                };
                if w_opt.is_some() || last_node.as_ref() == Some(v) {
                    for j in scan_pos..=i {
                        ...
                        for u in preds {
                            let u_label = graph.node(&u).cloned().unwrap_or_default();
        ...
        for i in south_pos..south_end {
            ...
            if graph.node(v).map(|n| n.dummy.is_some()).unwrap_or(false) {
                if let Some(preds) = graph.predecessors(v) {
                    for u in preds {
                        let u_node = graph.node(&u).cloned().unwrap_or_default();
        ...
        if let Some(_north) = prev {
            ...
            for (south_lookahead, v) in south.iter().enumerate() {
                if graph
                    ...
                    .unwrap_or(false)
                {
                    let predecessors = graph.predecessors(v).unwrap_or_default();
            ...
            if !ws.is_empty() {
                ...
                for i in lo..=hi {
                        ...
                        && !has_conflict(conflicts, v, &w)
                    {
                        ...
                        let root_w = root.get(&w).cloned().unwrap_or_else(|| w.clone());
...
pub fn horizontal_compaction(
    ...
    {
        let preds = |e: &str| block_g_ref.predecessors(e).unwrap_or_default();
        let mut pass1 = |elem: &str| {
            let in_es = block_g_ref.in_edges(elem).unwrap_or_default();
            if in_es.is_empty() {
                ...
                for e in in_es {
                    ...
                    let w = block_g_ref.edge_obj(&e).copied().unwrap_or(0.0);
                    let cand = xv + w;
                    if cand > max {
                        max = cand;
                    }
    ...
    {
        let succs = |e: &str| block_g_ref.successors(e).unwrap_or_default();
        let mut pass2 = |elem: &str| {
            let out_es = block_g_ref.out_edges(elem).unwrap_or_default();
            ...
            let bt = graph.node(elem).and_then(|n| n.border_type);
...
fn build_block_graph(
    ...
    let sep_fn = move |g: &Graph<GraphLabel, NodeLabel, EdgeLabel>, v: &str, u: &str| -> f64 {
        sep(node_sep, edge_sep, reverse_sep, g, v, u)
    };
    for layer in layering {
        ...
        for v in layer {
            ...
            if let Some(uu) = &u {
                if let Some(u_root) = root.get(uu) {
                    let prev_max = block_graph.edge(u_root, &v_root).copied().unwrap_or(0.0);
                    let s = sep_fn(graph, v, uu);
                    block_graph.set_edge(u_root.clone(), v_root.clone(), s.max(prev_max));
                }
            }
            u = Some(v.clone());
        }
...
pub fn position_x(graph: &Graph<GraphLabel, NodeLabel, EdgeLabel>) -> PositionMap {
    let layering = util::build_layer_matrix(graph);
    ...
    for vert in ["u", "d"] {
        ...
        for horiz in ["l", "r"] {
            ...
            let neighbor_fn = |v: &str| -> Vec<String> {
                if vert == "u" {
                    graph.predecessors(v).unwrap_or_default()
                } else {
                    graph.successors(v).unwrap_or_default()
                }
            };
            let (root, align) = vertical_alignment(&adjusted, &conflicts, neighbor_fn);
            let mut xs = horizontal_compaction(graph, &adjusted, &root, &align, horiz == "r");
            if horiz == "r" {
...
pub fn find_smallest_width_alignment(
    ...
    for xs in xss.values() {
        ...
        for (v, x) in xs {
            let hw = graph.node(v).map(|n| n.width).unwrap_or(0.0) / 2.0;
            max = max.max(x + hw);
            min = min.min(x - hw);
        }
        let width = max - min;
    ...
    for vert in ["u", "d"] {
        for horiz in ["l", "r"] {
            ...
            let is_align_to = xs.len() == align_to.len()
                && xs.iter().all(|(k, v)| align_to.get(k) == Some(v));
            if is_align_to {
                continue;
            }
            ...
            if delta != 0.0 && delta.is_finite() {
                let shifted: PositionMap = xs.into_iter().map(|(k, v)| (k, v + delta)).collect();
...
pub fn balance(xss: &HashMap<String, PositionMap>, align: Option<Align>) -> PositionMap {
    ...
    for v in ul.keys() {
        ...
        let mut xs: Vec<f64> = xss.values().filter_map(|m| m.get(v).copied()).collect();
