use crate::{Graph, alg};

/// Given a Graph, graph, this function returns all nodes that are part of a cycle. As there
/// may be more than one cycle in a graph this function return an array of these cycles,
/// where each cycle is itself represented by an array of ids for each node involved in
/// that cycle. Method [`alg::is_acyclic`] is more efficient if you only need to determine whether a graph has a
/// cycle or not.
/// Complexity: O(|V| + |E|).
///
/// @param graph - graph where to search cycles.
/// @returns cycles list.
pub fn find_cycles<GraphLabel, NodeLabel, EdgeLabel>(
	graph: &Graph<GraphLabel, NodeLabel, EdgeLabel>,
) -> Vec<Vec<String>> {
	alg::tarjan(graph)
		.into_iter()
		.filter(|components| {
			components.len() > 1
				|| (components.len() == 1
					&& graph.has_edge(
						components[0].clone(),
						components[0].clone(),
						None,
					))
		})
		.collect()
}
