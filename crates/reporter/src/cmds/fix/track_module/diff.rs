...
impl CompareBytes for CompareBytesIgnoreWhitespaceAmount {
	fn eq(&self, left: &[u8], right: &[u8]) -> bool {
		bytes_ignore_whitespace_amount(left)
			.eq(bytes_ignore_whitespace_amount(right))
	}
...
type HistogramEntry<'input> =
	(HashedWord<'input>, SmallVec<[LocalWordPosition; 2]>);
...
impl<'input> Histogram<'input> {
	...
	fn build_count_to_entries(
		&self,
	) -> BTreeMap<usize, Vec<&HistogramEntry<'input>>> {
...
fn collect_unchanged_words<C: CompareBytes, S: BuildHasher>(
	...
	let common_leading_len =
		iter::zip(left.hashed_words(), right.hashed_words())
			.take_while(|&(l, r)| comp.eq_hashed(l, r))
			.count();
	let left_hashed_words = left
		.hashed_words()
		.skip(common_leading_len);
	let right_hashed_words = right
		.hashed_words()
		.skip(common_leading_len);

	// Trim trailing common ranges (i.e. grow next unchanged region)
	let common_trailing_len =
		iter::zip(left_hashed_words.rev(), right_hashed_words.rev())
			.take_while(|&(l, r)| comp.eq_hashed(l, r))
			.count();
	...
	found_positions.extend(itertools::chain(
		...
		(1..=common_trailing_len)
			.rev()
			.map(|i| {
				(
					left.map_to_global(LocalWordPosition(
						left.ranges.len() - i,
					)),
					right.map_to_global(LocalWordPosition(
						right.ranges.len() - i,
					)),
				)
			}),
...
fn collect_unchanged_words_lcs<C: CompareBytes, S: BuildHasher>(
	...
	// the LCS.
	let Some(uncommon_shared_word_positions) = left_count_to_entries
		.values()
		.find_map(|left_entries| {
			let mut both_positions = left_entries
				...
				.filter_map(|&(word, left_positions)| {
					let right_positions =
						right_histogram.positions_by_word(*word, comp)?;
	...
	let (mut left_positions, mut right_positions): (Vec<_>, Vec<_>) =
		uncommon_shared_word_positions
			...
			.enumerate()
			.map(|(serial, (&left_pos, &right_pos))| {
				((left_pos, serial), (right_pos, serial))
			})
			.unzip();
	...
	collect_unchanged_words(
		found_positions,
		&left.narrowed(
			previous_left_position..LocalWordPosition(left.ranges.len()),
		),
		&right.narrowed(
			previous_right_position..LocalWordPosition(right.ranges.len()),
		),
		comp,
	);
}
...
impl<'input> ContentDiff<'input> {
	pub fn for_tokenizer<T: AsRef<[u8]> + ?Sized + 'input>(
				...
				.any(|input| input.is_empty())
		{
			...
			other_token_ranges =
				std::iter::repeat_n(vec![], other_inputs.len()).collect();
		} else {
	...
	fn with_inputs_and_token_ranges(
		...
		let other_sources = iter::zip(&other_inputs, other_token_ranges)
			.map(|(input, token_ranges)| {
				DiffSource::new(input, token_ranges, &comp)
			})
			.collect_vec();
	...
	pub fn by_word<T: AsRef<[u8]> + ?Sized + 'input>(
		...
		let mut diff = ContentDiff::for_tokenizer(
			inputs,
			find_word_ranges,
			CompareBytesExactly,
		);
	...
	fn hunk_at(
		...
		itertools::chain(
			...
			iter::zip(&self.other_inputs, &range.others)
				.map(|(input, r)| &input[r.clone()]),
		)
	}
		...
		for window in self.unchanged_regions.windows(2) {
			...
			for refined in &refined_diff.unchanged_regions {
				...
				let new_others = iter::zip(&refined.others, &previous.others)
					.map(|(refi, prev)| {
						(refi.start + prev.end)..(refi.end + prev.end)
					})
					.collect();
...
impl<'input> Iterator for DiffHunkIterator<'_, 'input> {
	...
	fn next(&mut self) -> Option<Self::Item> {
		self.ranges.next_with(
			...
			|previous, current| {
				let contents: DiffHunkContentVec = self
					.diff
					.hunk_between(previous, current)
					.collect();
				debug_assert!(
					contents
						.iter()
...
impl Iterator for DiffHunkRangeIterator<'_> {
	...
	fn next(&mut self) -> Option<Self::Item> {
		self.next_with(
			|previous| {
				let ranges = itertools::chain(
					iter::once(&previous.base),
					&previous.others,
				)
				.cloned()
				.collect();
	...
	fn unchanged_ranges(
		...
		positions
			...
			.map(|(left_pos, right_pos)| {
				(left.range_at(left_pos), right.range_at(right_pos))
			})
			.collect()
	}
