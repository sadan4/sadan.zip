...
use crate::err::printer::{
	diagnostic_chain::{DiagnosticChain, ErrorKind},
	hl::{Highlighter, MietteHighlighter},
};
...
impl GraphicalReportHandler {
	...
	pub fn with_show_related_as_nested(
		mut self,
...
impl GraphicalReportHandler {
	...
	fn render_causes(
		...
		if let Some(mut cause_iter) = diagnostic
			...
			.or_else(|| {
				diagnostic
					.source()
					.map(DiagnosticChain::from_stderror)
			})
			.map(|it| it.peekable())
		{
			while let Some(error) = cause_iter.next() {
				...
				match error {
					ErrorKind::Diagnostic(diag) => {
						...
						inner_renderer
							.render_report_inner(&mut inner, diag, src)?;
	...
	fn render_related(
		...
		let related = diagnostic
			.related()
			.map_or_default(Itertools::collect_vec);
		if !related.is_empty() {
	...
	fn render_snippets(
		...
		let mut labels = match diagnostic.labels() {
			Some(l) => l.collect_vec(),
			None => Vec::new(),
		};
		if labels.is_empty() {
			return Ok(());
		...
		for right in labels.iter().cloned() {
			let right_conts = match source.read_span(
				...
				Err(err) => {
					writeln!(
						f,
						"  [{} `{}` (offset: {}, length: {}): {:?}]",
						"Failed to read contents for label"
			...
			if left_conts.line() + left_conts.line_count() >= right_conts.line()
			{
				...
				let new_end = left_end.max(right_end);
	...
	fn render_context(
		...
		let mut highlighter_state = self
			.highlighter
			.start_highlighter_state(&*contents);
		...
		for line in &lines {
			...
			max_gutter = max_gutter.max(num_highlights);
		}
	...
	fn render_line_gutter(
		...
		let applicable = highlights
			.iter()
			.filter(|hl| line.span_applies_gutter(hl));
		let mut arrow = false;
	...
	fn render_line_text(
		...
		for (c, width) in text
			.chars()
			.zip(self.line_visual_char_width(text))
		{
			if c == '\t' {
				for _ in 0..width {
					f.write_char(' ')?;
	...
	fn render_single_line_highlights(
		...
		let vbar_offsets: Vec<_> = single_liners
			...
			.map(|hl| {
				...
				let start = self
					.visual_offset(line, byte_start, true)
					.max(highest);
				let end = if hl.len() == 0 {
					start + 1
				} else {
					self.visual_offset(line, byte_end, false)
						.max(start + 1)
				};
	...
	fn get_lines<'a>(
		...
		let mut offset: usize = context_data.span().offset();
...
fn display_link(href: &str, txt: Option<&str>) -> String {
	format!(
		"\u{1b}]8;;{href}\u{1b}\\{}\u{1b}]8;;\u{1b}\\",
		txt.unwrap_or(href)
	)
}
