...
impl BuildFilter {
	pub fn choose(
		&self,
		mut from: impl Iterator<Item = BundleMetadata>,
	) -> Option<BundleMetadata> {
		match self {
			Self::Latest => from.max_by_key(|f| f.first_seen),
			Self::Number(build_number) => {
				from.find(|build| build.build_number == *build_number)
			}
			Self::Hash(build_hash) => {
				from.find(|build| build.build_hash == *build_hash)
			}
		}
	}
}
...
async fn fetch_build_from_disk(path: &Path) -> Result<FullBundle> {
	...
	let ret =
		read_struct(&*data).context("Failed to parse bundle from disk.")?;
	debug!("Read bundle from disk");
	Ok(ret)
}

async fn fetch_build_from_server(
	opts: &FetchOpts,
	bar: &Stage,
) -> Result<FullBundle> {
	...
	let full_build =
		reqwest::get(format!("{backend_url}/build/{}/full", build.build_hash))
			.await?;
